...
class Upload:
    def __init__(
        ...
        self.driver = webdriver.Firefox(
            firefox_profile=profile, options=options, executable_path=executable_path, firefox_binary=r"C:\Program Files\Mozilla Firefox\firefox.exe"
        )
    ...
    def upload(
        ...
        if description:
            ...
            container = modal.find_element_by_css_selector(DESCRIPTION_CONTAINER)
        ...
        if not short:
            try:
                self.click(self.driver.find_element_by_css_selector(MONITIZATION_BOX))
                self.click(self.driver.find_element_by_css_selector(MONITIZATION_ON))
                self.click(self.driver.find_element_by_css_selector(MONITIZATION_DONE))
            except:
                pass

        self.click_next(modal)
        try:
            self.click(self.driver.find_element_by_css_selector(AD_SUITABLE))
            self.click(self.driver.find_element_by_css_selector(AD_SUITABLE_SUBMIT))
        except:
            pass
        ...
        try:
            public_main_button = modal.find_element_by_name(UNLISTED_BUTTON)
        except:
            time.sleep(20)
            self.click_next(modal)
            public_main_button = modal.find_element_by_name(UNLISTED_BUTTON)
